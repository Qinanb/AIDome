# Description: 知识库管理API，使用硅基流动Embedding模型和本地FAISS向量存储库进行知识库的创建、删除、文件上传、文件删除、搜索等操作。
import os
import shutil
...
embedding = OpenAIEmbeddings(
            api_key="sk-tjyriwuwdifftbdqduwdcwekhnrhdjsmepwrefqdrmbjgtem",
...
async def create_kb(
        kb_name: str = Body(..., min_length=1, pattern=r"^\S.*$", description="知识库名称"),
    ...
    if not kb_name:
        raise HTTPException(status_code=400, detail="知识库名称不能为空或纯空格")
